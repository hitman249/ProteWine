...
<script lang="ts">
  ...
  import ContainersMenu from '../modules/containers';
  ...
  const menu: ContainersMenu = new ContainersMenu();
  let items: MenuItem[] = menu.getItems();

  export function getMenu(): ContainersMenu {
    return menu;
  }
  ...
  function getStickerType(template: ValueLabels): StickerType {
    if (template === ValueLabels.CONTAINER) {
      return StickerType.CONTAINER;
    }

    if (template === ValueLabels.MANAGE) {
      return StickerType.CONTAINER;
    }

    if (template === ValueLabels.GAME) {
      return StickerType.GAME;
    }

    return StickerType.ITEM;
  }
...
<div class="content" style="{style}">
    ...
    {#if horizontalList}
      {#each items as item, index}
        ...
        <ListPreloader
          ...
          delta={-50}
          headersDummy={1}
          paddingIndent={item.template === ValueLabels.CONTAINER ? -80 : 0}
          marginIndent={item.template === ValueLabels.CONTAINER ? -10 : 0}
          itemCenter={false}
          horizontal={false}
          itemSize={item.template === ValueLabels.CONTAINER ? ContainersMenu.ITEM_HEIGHT : Menu.ITEM_HEIGHT}
          itemSpace={item.template === ValueLabels.CONTAINER ? ContainersMenu.ROOT_ITEM_HEIGHT : Menu.ROOT_ITEM_HEIGHT}
          extendItemClass="vertical-item"
          type={getStickerType(item.template)}
        />
      {/each}
    {/if}
  </div>
  ...
  {#if innerListItem}
    ...
    {@const type = getStickerType(innerListItem.template)}
    {@const isContainer = (StickerType.GAME === type || StickerType.CONTAINER === type)}
    ...
    <div class="inner-list" class:list-only-active={isSelectList} class:animate-items-opacity={selectListItems}>
      <ListPreloader
        ...
        model={innerListItem}
        delta={0}
        headersDummy={isContainer ? 2 : 3}
        paddingIndent={isContainer ? -70 : -100}
        itemSize={isContainer ? ContainersMenu.ITEM_HEIGHT : Menu.ITEM_HEIGHT}
        itemSpace={isContainer ? 80 : 50}
        itemCenter={true}
        horizontal={false}
        extendItemClass="vertical-item"
        {type}
